...
- [x] Remove the `.finalImageGhost` placeholder and rebuild the reveal around one box (frame = placeholder + photo container), so the painting's footprint is claimed from data instead of from a stand-in element that has to be removed again
- [x] Stop `.canvas.revealed` reserving the hidden topbar/palette bands (kills the 39px post-reveal slide from `--topbar-h`'s ResizeObserver, keeps `env(safe-area-inset-top)`)
- [x] Pin the revealed stack's vertical placement (`pinRevealedStack`) so the trivia plaque growing when its details land can't slide the finished painting up
- [x] Make the revealed painting big again (largest on-ratio box fitting the wrap width and a 60dvh ceiling, was capped at 480px wide), fill the frame with no letterbox, and make the hero's 12px frame border actually apply over `.museumFrame`'s 8px
- [x] Fix the last reveal jump (~20px at the end of the photo's fade, caught in `debug/jump.mov`): WebKit resolves `aspect-ratio` on a `content-box` element against width-minus-border, disagreeing with the flex automatic minimum that `.skeleton`'s `overflow: hidden` had been suppressing — carry the ratio on the border box, pin `min-height: 0`, and let `object-fit: cover` (`.finalImage.exact`) absorb the resulting border-sized ratio offset
