...
- This machine has full Xcode (26.6) + CocoaPods set up, and the app has been built and run on a physical iPhone via a free Apple ID (Personal Team) — see ROADMAP.md's 2026-09-10-ish entry. TestFlight/App Store distribution still needs a paid Apple Developer Program enrollment (not yet done), but local iOS builds/testing already work. Android has no SDK/Android Studio installed here at all — `cap sync` (which just copies web assets and doesn't invoke either native toolchain) works regardless, but actually building/running the Android app requires setting up Android Studio + the Android SDK from scratch.
